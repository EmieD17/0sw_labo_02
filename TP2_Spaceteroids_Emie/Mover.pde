class Mover extends GraphicObject {
  ...
  Mover (PVector loc, PVector vel) {
    // intérieur gauche
    if(loc.x < width / 2 && loc.x > (width * 0.4)) 
    {
      loc.x -= (width * 0.2);
    }
    // intérieur droite
    if(loc.x > width / 2 && loc.x < (width * 0.6)) 
    {
      loc.x += (width * 0.2);
    }
    // intérieur haut
    if(loc.y < height / 2 && loc.y > (width * 0.4)) 
    {
      loc.x -= (width * 0.2);
    }
    // intérieur bas
    if(loc.y > width / 2 && loc.y < (width * 0.6)) 
    {
      loc.x += (width * 0.2);
    }
